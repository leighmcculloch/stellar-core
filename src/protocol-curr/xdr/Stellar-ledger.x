...
#ifdef PROTOCOL_CHANGE_BY_NAME
// Name of a protocol change that validators vote on. Protocol changes are
// identified by name rather than by the protocol version number they would
// produce, so that several changes can be proposed concurrently and whichever
// one is accepted first becomes the next protocol version.
typedef string ProtocolChangeName<64>;

struct LedgerHeaderExtensionV2
{
    // Names of the protocol changes that have been activated by validator
    // vote, sorted lexicographically. Each accepted change incremented
    // `ledgerVersion` by one, so the protocol version counts how many changes
    // have been activated rather than identifying any particular one.
    ProtocolChangeName activatedProtocolChanges<128>;
...
#ifdef PROTOCOL_CHANGE_BY_NAME
    ,
    LEDGER_UPGRADE_PROTOCOL_CHANGE = 8
#endif // PROTOCOL_CHANGE_BY_NAME
};
...
case LEDGER_UPGRADE_PROTOCOL_CHANGE:
    // Activate the named protocol change and increment `ledgerVersion` by one.
    // The name identifies the change; the resulting protocol version depends
    // only on how many changes have already been activated.
